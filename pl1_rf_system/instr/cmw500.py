...
import os, sys, time,  logging, math, traceback, re
...
from pl1_rf_system.instr.cmw_gen_rf import gprf_gen
from pl1_rf_system.instr.cmw_gen_rf import gprf_meas
...
class CmwControl:
    ...
    def __init__(self,name, ip_addr,verdictObj=Verdict(),NoTester=False):
        if not name.upper() in self.CMW_NAME_L:
            err_msg = ('must use -instr_name [ %s ]' % ' | '.join(sorted(self.CMW_NAME_L)))
            raise ExCmw(err_msg)
        ...
        command_completed = self.FAIL

        if not NoTester:
            MAX_NUM_OF_INSTR_INIT_ATTEMPTS = 3
            attempt_idx = 0
            while (attempt_idx < MAX_NUM_OF_INSTR_INIT_ATTEMPTS) :
                attempt_idx += 1
                try:
                    self.dev = rohde_and_schwarz_CMW500(ip_addr, timeout=10)
                    logging.info("Connected to CMW500: cmwname=%s, cmwip=%s" % (self.name, ip_addr))
                    logging.info("Attempt %s of %s" %(attempt_idx, MAX_NUM_OF_INSTR_INIT_ATTEMPTS))
                except Exception:
                    errMsg=("CMW500 CONNECTION FAILURE: cmwname=%s, cmwip=%s" % (self.name, ip_addr))
                    raise ExCmw500(errMsg)
                self.read("*cls")
                command_completed = self.waitForCompletion()
                if command_completed == self.SUCCESS:
                    ...
                    logging.info("Disconnected from CMW500: cmwname=%s, cmwip=%s" % (self.name, ip_addr))
                    self.dev.disconnect()

        if command_completed == self.FAIL:
            errMsg=("CMW500 COMMAND INIT FAILURE: cmwname=%s, cmwip=%s" % (self.name, ip_addr))
            raise ExCmw500(errMsg)

        self.resultsFile = ""

        self.wcdma_tx = Wcdma_meas(self)

        self.lte_tx = Lte_meas(self)

        self.gprf_gen = gprf_gen(self)

        self.gprf_meas = gprf_meas(self)

        self.verdictObj = verdictObj

        self.NoTester = NoTester

        self.reliabilitydic = {0:"OK",1:"Measurment Timeout",2:"Capture Buffer Overflow",3:"Input Overdriven",
                               4:"Input Underdriven",6:"Trigger Timeout",7:"Acquisition Error",8:"Sync Error",
                               9:"Uncal",15:"Ref Freq Error",16:"RF Not Avail", 17:"RF Level Not Settled", 18: "RF Freq not Settled",
                               19:"Call not established"}

        self.FWVersionRead = False
    ...
    def set_verdictObj (self,verdictObj):
        self.verdictObj = verdictObj
        self.wcdma_tx.set_verdictObj(verdictObj)
        self.lte_tx.set_verdictObj(verdictObj)
    ...
    def waitForCompletion(self, timeout=30):
        ...
        POLL_INTERVAL = 1
        command_status = self.FAIL
        while (num_iter < NUM_ITER_MAX):
            completed=(self.read("*OPC?") == "1")
            if completed:
                command_status = self.SUCCESS
                return command_status
            num_iter += 1
            time.sleep(POLL_INTERVAL)
            if num_iter == NUM_ITER_MAX:
                loggerCmw.error('Did not receive the expected response')
                return command_status
    ...
    def write(self, command):
        loggerCmw=logging.getLogger(__name__ + 'cmu.write')
        loggerCmw.debug ("   %s write command \"%s\"" % (self.name, command))
    ...
    def read(self, command):
        ...
        loggerCmw=logging.getLogger(__name__ + 'cmw.read')
        ...
        rel_ind = 0
        if len(readingshort) > 10:
            #Check reliability indicator if present -- format is integer followed by comma
            if self.is_int(readingshort[0:2]) and readingshort[2] == ",":
                rel_ind = int(readingshort[0:2])
            elif self.is_int(readingshort[0]) and readingshort[1] == ",":
                rel_ind = int(readingshort[0])
            if rel_ind != 0:
                try:
                    rel_ind_string = self.reliabilitydic[rel_ind]
                except KeyError:
                    rel_ind_string = "Unknown CMW Issue - Check manual for reliability indicator %d" % rel_ind
                self.verdictObj.RecordError("CMW Failure - %s" % rel_ind_string)
    ...
    def wait_response(self, scpi_query_cmd="", exp_rsp="", timeout = 5):
        loggerCmw = logging.getLogger(__name__ + 'cmu.wait_response')

        if scpi_query_cmd == "":
            loggerCmw.error("no scpi command query")
            return False
        else:
            loggerCmw.debug("Waiting for response: %s" %exp_rsp)
            start_time= int(time.time())
            elapsed_time = 0
            while elapsed_time <= timeout:
                status = self.read(scpi_query_cmd)
                loggerCmw.debug("response is: %s " %status)
                if status == exp_rsp:
                    loggerCmw.info("Expected response received within %s secs"  % timeout)
                    return True
                time.sleep(1)
                cum_time = int(time.time())
                elapsed_time = cum_time - start_time
                loggerCmw.debug (elapsed_time)

            loggerCmw.error("Expected response not received within %s secs"  % timeout)
    ...
    def set_scenario(self):
        ...
        self.write('ROUTe:WCDMa:MEAS:SCENario:SALone RF1C, RX1')
    ...
    def set_wcdma_general_meas_settings(self, freqMHz=1950, cable_loss_dB=0):

        self.set_rf_settings_freqMHz(freqMHz=freqMHz)

        # external attaenuation setting
        self.set_rf_settings_ext_att(atten_dB=cable_loss_dB)

        # set the expected nominal power
        self.set_rf_settings_exp_power(power_dBm=0)

        self.set_rf_settings_user_margin(value_dB=12)


    def set_rf_settings_freqMHz(self, freqMHz):

        freqHz = int(freqMHz * 1e6)

        self.write('CONFigure:WCDMA:MEAS:RFSettings:FREQuency %s' %freqHz)

    def set_rf_settings_ext_att(self, atten_dB):

        self.write('CONFigure:WCDMa:MEAS:RFSettings:EATTenuation %s' %atten_dB)

    def set_rf_settings_exp_power(self, power_dBm):

        # expected nominal power

        self.write('CONFigure:WCDMa:MEAS:RFSettings:ENPower %s' %power_dBm)

    def set_rf_settings_user_margin(self, value_dB):

        self.write('CONFigure:WCDMA:MEAS:RFSettings:UMARgin %s' %value_dB)
    ...
    def get_instr_hsdpa_bler(self, carrier=1):
        """
    ...
    def get_ack_trans_meas(self, carrier=1):
    ...
    def get_hsdpa_ack_meas(self, numSubframes=2000):
        ...
        self.get_ack_trans_meas(carrier=1)

        if self.dc_hsdpa:

            self.get_ack_trans_meas(carrier=2)

        numMeasuredFrames = self.get_measured_subframes()
        ...
        blerVal = self.get_instr_hsdpa_bler(carrier=1)

        self.set_hsdpa_bler(blerVal, carrier=1)

        if self.dc_hsdpa:

            blerVal = self.get_instr_hsdpa_bler(carrier=2)
    ...
    def get_sw_version(self):
        ...
        if not self.FWVersionRead and not self.NoTester:
            self.cmwswinfo=self.read("SYSTem:BASE:OPTion:VERSion?")
            self.FWVersionRead = True

        if self.NoTester:
            self.cmwswinfo="NO CMW"

        return self.cmwswinfo

    def checkSwVersion(self, msg_str):

        #CMW_WCDMA_Meas,V3.2.60

        check_l   = {'CMW_BASE':(3, 2, 50), 'CMW_WCDMA_Sig':(3, 2, 80),
                     'CMW_LTE_Meas':(3, 2, 81)}
        ...
        for k,v in check_l.iteritems():
            ...
            if tmp.search(msg_str):
                ...
                if ((x>v[0]) or (x==v[0] and y>v[1]) or (x==v[0] and y==v[1] and z>=v[2])):
                    ...
                    verdict=1
                    errMsg = ("Incorrect CMW SW version %s. Required v%s.%s.%s or later" % (check_str, v[0], v[1], v[2]))
                    self.verdictObj.RecordError(errMsg)
                    errMsg += ("\nTested on CMW_LTE_Meas,X3.2.81.1 beta version\n")
                    errMsg += ("Please skip this test or update CMW software!")
    ...
    def setup_3g_tx_test(self, cable_loss_dB=0):
        ...
        self.wcdma_tx.set_scenario()

        self.wcdma_tx.set_default_general_settings()

        # external attaenuation setting
        self.wcdma_tx.set_rf_ext_att_in(atten_dB=cable_loss_dB)

        # set the expected nominal power
        self.wcdma_tx.set_rf_exp_power(power_dBm=0)

        self.wcdma_tx.set_ue_signal_info()

        self.wcdma_tx.set_up_meas_control()

        #self.wcdma_tx.get_evm_meas()

    def setup_4g_tx_test(self, cable_loss_dB=0, bwMHz=5, band=1):
        ...
        self.lte_tx.set_scenario()

        self.lte_tx.set_default_general_settings()

        self.lte_tx.set_band(band=band)

        self.lte_tx.set_rf_ext_att_in(atten_dB=cable_loss_dB)

        self.lte_tx.set_rf_exp_power(power_dBm=5)

        self.lte_tx.set_mixer_level_offset_dB(val=0)

        self.lte_tx.set_up_meas_control()

        self.lte_tx.set_channel_bw_MHz(bwMHz=bwMHz)

    def setup_rf_tone (self,cable_loss_dB=0):
        ...
        self.gprf_gen.set_scenario()

        self.gprf_gen.set_default_general_settings()

        self.gprf_gen.set_rf_ext_att_in(atten_dB=cable_loss_dB)

        self.gprf_gen.set_rf_generator_state(state='ON')

    def setup_rf_power_meas (self,cable_loss_dB=0):
        ...
        self.gprf_meas.set_scenario()

        self.gprf_meas.set_default_general_settings()

        self.gprf_meas.set_rf_ext_att_in(atten_dB=cable_loss_dB)
...
if __name__ == '__main__':
    ...
    try:
        ...
        cmw500=CmwControl(name="cmw500", ip_addr=cmwip)

        if rat.upper() == '3G':

            cmw500.setup_3g_tx_test(freq=freqMHz, cable_loss_dB=cable_loss_dB)

        elif rat.upper() == 'LTE':

            cmw500.setup_4g_tx_test(freq=freqMHz,
                                    cable_loss_dB=cable_loss_dB,
                                    bwMHz=bwMHz)

            cmw500.lte_tx.get_evm_meas(num_cycles=10)

        elif rat.upper() == 'RF':
            cmw500.setup_rf_power_meas()
