 ...
 ************************************************************************************************/
  ...
  *This document describes the Nvidia API functions in the Updater.dll which is the dynamic link library for downloader tools.\n
  ...
  *  1.Nvidia API functions and handle structure for multiple instances in the dll\n
  ...
  *
  *
  *Below is the section for how to generate a downloader tool/UpdaterLib.\n
  *We are using Visual Studio to develop downloader and UpdaterLib. So, you can refer to this section for how to generate
  ...
  *The downloader.exe is an .exe that embed UpdaterLib as static libraries.\n
  *This project rebuild UpdaterLib static libray
  ...
  *can be generated.\n
  *
  * @link build How to generate a downloader tool
  * @endlink \n
  *
  ...
  * @page build How to generate a downloader tool
  * <b>For Windows 7/8/RT</b>:
  *	 Open the .sln project files with Visual Studio and then compile for the wanted platform\n
  *		(tested with Visual Studio 2012 and 2013)
  * 
  * <b> For Linux x86</b>:\n
  *	Install the g++ compiler if not available then use the makefile located	inside build/ directory\n
  *		\code make \endcode \n
  *
  * <b>For Linux ARM (aka Android)</b>:
  * Download the <a href="https://developer.android.com/tools/sdk/ndk/index.html"> Android NDK (Native Development Kit) </a> and add it to your PATH \n then call the makefile inside the build/ directory as follow\n
  *	\code make arch=ARM \endcode \n\n
  *
  * The binaries will be generated inside bin/ directory
  */
...
extern "C"{
#endif
ICERA_API extern const int ErrorCode_Error;
ICERA_API extern const int ErrorCode_Success;
ICERA_API extern const int ErrorCode_FileNotFound;
ICERA_API extern const int ErrorCode_DeviceNotFound;
ICERA_API extern const int ErrorCode_AT;
ICERA_API extern const int ErrorCode_DownloadError;
ICERA_API extern const int ErrorCode_ConnectionError;
ICERA_API extern const int ErrorCode_InvalidHeader;

ICERA_API extern const int ProgressStep_Error;
ICERA_API extern const int ProgressStep_Init;
ICERA_API extern const int ProgressStep_Download;
ICERA_API extern const int ProgressStep_Flash;
ICERA_API extern const int ProgressStep_Finalize;
ICERA_API extern const int ProgressStep_Finish;


typedef unsigned long UPD_Handle;

ICERA_API extern const int status_success;
ICERA_API extern const int status_error;
ICERA_API extern const int status_found;
ICERA_API extern const int status_not_found;
ICERA_API extern const int status_no_enough_memory;
ICERA_API extern const int status_bad_parameter;
#ifdef ICERA_EXPORTS
}
#endif
...
* @retval   1 : Success
*/
ICERA_API bool Write(UPD_Handle handle, void *buf, int sz);
...
/**
 * @details This function returns two versions of port list which are connected to the host 
 *               After the lists are no longer required call FreePortList to release the lists 
 * @param filter : which port types (Serial/Winusb/...) and functions (MODEM/AT/...) to be detected
 * @param port_list : the string array of detected port names 
 * @param port_detail_list : the string array for full name of ports detected
 * @retval number of ports detected
 */
...
/**
* @details Per Handle setting for checking if autodection is used\n
* @param   check => 0 : don't check; 1 : check if mode restore successfully
*/
ICERA_API void SetAutodetection(UPD_Handle handle, bool check);

/**
* @details Per Handle setting for checking if standalone loader is used\n
* @param   standalone_loader => AT%MODE=1,3 : don't standalone_loader => AT%MODE=1 
*/
...
/**
* @details Compare the arch hash values\n
* @param   arch_hash_values: the firmware file hash values
*          is_same => 0 : hash different, 1 : hash the same
*/
ICERA_API int CompareArchHashValues(UPD_Handle handle, const char* arch_hash_values, bool *is_same);

/**
* @details Retrieve the Firmware Id\n
* @param   buffer: the char buffer to store the firmware id, can be NULL
*          buffer_size: the size of the buffer parameter
*          required_size: required buffer size to store firmware id. Shouldn't be NULL.
*/
