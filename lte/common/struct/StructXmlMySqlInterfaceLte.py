...
# GLOBAL OBJECTS
# ********************************************************************
class StructXmlMySqlInterfaceLte(StructXml):
    
    FILEXML_MYSQL_TEMPLATE=os.sep.join(os.environ['PL1TESTBENCH_ROOT_FOLDER'].split(os.sep)[:]+['lte','common', 'struct', 'template', 'structxml_mysqldb_lte_template.xml'])
    
    def __init__(self, struct_name):    
        logger=logging.getLogger('StructXmlMySqlInterfaceLte.__init__')
        
        self.STRUCTNAME  = struct_name
        self.FIELDNAME_L = []
        
        # Create substructures
        self.NODE_L=['table_platforms', 
                     'table_testinfos', 
                     'table_testruns', 
                     'table_lte_params', 
                     'table_lte_results']
        ...
        for node in self.NODE_L:
            setattr(self, node, eval("StructXml(xmlfile=self.FILEXML_MYSQL_TEMPLATE, struct_name=(struct_name + '.' + node), node_name=node)"))
