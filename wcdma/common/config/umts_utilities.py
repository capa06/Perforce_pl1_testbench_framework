...
dl_UARFCN_table={
 ...
 15:[735, 877.4, 887.6],
 16:[-109, 793.4, 818.6],
 17:[1326, 1498.4, 1508.4],
 18:[2580, 3512.4, 3587.6],
 19:[910, 1932.4, 1992.6],
 20:[-291, 861.4, 891.6]
}
...
def default_dl_uarfcn(band):
    try:
        ...
        #print "mid band for band %s not defined" %band
        #print "current script supports the following bands %s" %uarfcn_mid_range.keys()
        return (-1)

    return uarfcn


def default_uarfcn(band):
    # included for backwards compatibility
    return default_dl_uarfcn(band)
...
def Range_EARFCN(band):
    return (min_dl_UARFCN(band), max_dl_UARFCN(band))


def main():

    get_all_supported_dl_freq(band=2)
    get_all_supported_ul_freq(band=2)
    uarfcn_minmax = Range_EARFCN(2)
    print uarfcn_minmax

    uarfcn_intraho_l=range(uarfcn_minmax[0],uarfcn_minmax[1]+1)
    print uarfcn_intraho_l

    # minimum number of uarfcn to test
    min_num_uarcn = 2

    uarcn_step_size = len(uarfcn_intraho_l)/min_num_uarcn

    selected_uarfcn_l = uarfcn_intraho_l[::uarcn_step_size]

    print len(selected_uarfcn_l)

    print selected_uarfcn_l

    bot = min_dl_UARFCN(band=2)

    mid = default_dl_uarfcn(band=2)

    top = max_dl_UARFCN(band=2)

    add_uarfcn_list = [bot, mid, bot, top, bot, mid, top, mid]

    print add_uarfcn_list + selected_uarfcn_l

    band = 2
    uarfcn = get_dl_uarfcn(freq=2500, band=7)
    print uarfcn
    ...
    for band_idx in dl_UARFCN_table.keys():
        uarfcn = default_uarfcn(band_idx)
        if uarfcn == -1:
            continue
        print "band: %s, uarfcn: %s, freq (MHz):%s" %(band, uarfcn, get_umts_dl_freq(uarfcn))


    min_uarfcn = min_dl_UARFCN(band)
    print min_uarfcn
    max_uarfcn = max_dl_UARFCN(band)
    print max_uarfcn
    print get_dl_uarfcn(2112.6, band)
    print min_uarfcn
    print max_uarfcn
    uarfcnList = range(min_uarfcn, max_uarfcn, 1)
    print uarfcnList
