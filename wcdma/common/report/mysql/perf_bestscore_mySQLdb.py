...
import os, sys, logging, time, sqlite3, re, bz2
import traceback
...
# Icera decoders
#icera_aes = os.sep.join(os.environ['PL1_WCDMA_TEST_ROOT'].split(os.sep)[:]+['common', 'icera', 'icera-aes'])
...
# =============================================================================
def DB_mySQL_destroy(dbname):
    logging.debug("DB_mySQL_destroy()...")
    db_h=results_mySQLdb(dbname)
    db_h.destroy()
    del db_h

def DB_mySQL_CheckPermission(host, dbname, uid, pwd):

    logging.debug("db_params: name %s, uid %s, host %s" % (dbname, uid, host))

    try:
        # Get database connection object instance
        db_h=results_mySQLdb(host, dbname, uid, pwd)
        ...
        logging.debug("Available access to DB: %s@%s for User: %s is %s." %(dbname, host, uid, res))

        db_h.disconnect()
        return res

    except :
        logging.warning("ACCESS to Database is not available.")
        print traceback.format_exc()
        return None


def DB_mySQL_import_from_file_meas(dbname, filename):

    logging.debug("DB_mySQL_import_from_file_meas()...")
    logging.debug("DB_mySQL_import_from_file_meas()...FILE %s" % filename)
...
def DB_mySQL_wcdma_import_from_file_meas(filename, testType, host,
                                         dbname, uid, pwd, config_pwr_meas):

    logging.debug("DB_mySQL_wcdma_import_from_file_meas()...")
    logging.debug("DB_mySQL_wcdma_import_from_file_meas()...FILE %s" % filename)

    # Get database connection object instance
    db_h=results_mySQLdb(host, dbname, uid, pwd, config_pwr_meas)
    ...
    elif testType == 'HSPA_FADING_PERF':
        keywords_l=['TESTID', 'RFBAND', 'UARFCN', 'CHTYPE', 'DATARATE', 'SNR', 'POWER', 'TXANTS', 'SCHEDTYPE', 'CPICH_Power', 'HS-PDSCH_Power', 'NSF', 'REF Tput(Mbps)', 'Best Score Tput (Mbps)', 'DL_TPUT(Mbps)', '%_Ref_Thput', '%_Best_Thput', 'DL_BLER', 'TOL(%)', 'CQI', 'SENT(%)', 'ACK(%)', 'NACK(%)', 'DTX(%)', 'DL VERDICT', 'Imin[mA]', 'Iavrg[mA]', 'Imax[mA]', 'Ideviation', 'PWRmin[mW]@3.8V', 'PWRavrg[mW]@3.8V', 'PWRmax[mW]@3.8V']
    else:
        logging.warning("Unknown test type %s. TestID %s SKIPPED" % testType)
        # Close database connection
        db_h.disconnect()
        return
    ...
    if 1:
        ...
        for idx in range(total_entries):
            if testType == 'BLER_PERF':
                ...
                w_param_id=db_h.add_wcdma_hspa_fading_test_param_set(testinfo_id=testinfo_id, idx=idx, table_entries=table_entry)
                result_id=db_h.add_wcdma_hspa_fading_test_result(testrun_id, platform_id, w_param_id, idx, table_entry)
                logging.info("SELECTED: result_id:%d, %s",result_id, (testrun_id, platform_id, w_param_id))
            else:
...
def DB_mySQL_export_to_file(dbname, equery, filename):

    logging.debug("DB_mySQL_export()...")
    logging.debug("DB_mySQL_export()...FILE %s" % filename)

    # Check database
    if not os.path.isfile(dbname):
        logging.warning("DB_mySQL_export_to_file():: database %s not found. Export ABORTED" % dbname)
        return -1
...
def DB_mySQL_export_bestscore(host, dbname, uid, pwd, equery):

    logging.debug("DB_mySQL_export_bestscore()...")
...
#    selectout_entry_frmt='%s, '*(len(selectout_header_l)-1)+'%s\n'
    ...
    try:
        ...
        if db_h.cursor:
            ...
            logging.debug("DB_mySQL_export_bestscore(), no entry found in database %s" % dbname)
    except TypeError:
        logging.error("DB_mySQL_export_bestscore(), SELECT failure")
        return (None, None, None)
    else:
        logging.debug("DB_mySQL_export_bestscore() completed")
        # Close database connection
        db_h.disconnect()
...
def DB_mySQL_SetQueryBestScore(modeminfo, testparams, varname):
    logging.debug(">>>> DB_mySQL_SetQueryBestScore()...")
...
def DB_mySQL_SelectBestScore(host, dbname, uid, pwd, modeminfo, testparams):
    logging.debug(">> DB_mySQL_SelectBestScore()...")
    dlthr_equery=DB_mySQL_SetQueryBestScore(modeminfo, testparams, 'dlthr')
    dlthr_branch, dlthr_clnum, dlthr_score = DB_mySQL_export_bestscore(host=host, dbname=dbname, uid=uid, pwd=pwd, equery=dlthr_equery)
    ulthr_equery=DB_mySQL_SetQueryBestScore(modeminfo, testparams, 'ulthr')
    ulthr_branch, ulthr_clnum, ulthr_score = DB_mySQL_export_bestscore(host=host, dbname=dbname, uid=uid, pwd=pwd, equery=ulthr_equery)
    return (dlthr_branch, dlthr_clnum, dlthr_score), (ulthr_branch, ulthr_clnum, ulthr_score)



def DB_mySQL_wcdma_hspa_fading_SetQueryBestScore(modeminfo, testparams, varname, rtype = 'DEFAULT'):
    logging.debug(">>>> DB_mySQL_SetQueryBestScore()...")

    #param_list = [self.testID, rfband, uarfcn, chtype, data_rate, snr, rfpower, txants, schedtype, cpich_power, hspdsch_power]
    if rtype == 'DEFAULT':
        branch      = GetBranch(modeminfo)
        platform    = GetPlatform(modeminfo)
    else:
    ...
    uarfcn          = testparams[2]
    chtype          = testparams[3]
    datarate        = testparams[4]
    snr             = testparams[5]
    rfpower         = testparams[6]
    txants          = testparams[7]
    schedtype       = testparams[8]
    cpich_power     = testparams[9]
    hspdsch_power   = testparams[10]
    ...
    if 0:
        ...
        print  "branch        : %s" % branch
        print  "platform      : %s" % platform
        print  "rfband        : %s" % rfband
        print  "uarfcn        : %s" % uarfcn
        print  "snr           : %s" % snr
        print  "chtype        : %s" % chtype
        print  "datarate      : %s" % datarate
        print  "rfpower       : %s" % rfpower
        print  "txants        : %s" % txants
        print  "cpich_power   : %s" % cpich_power
        print  "schedtype     : %s" % schedtype
        print  "hspdsch_power : %s" % hspdsch_power


    equery_bestscore=re.sub('[ |\t|\n]+',' ', r"""SELECT branch,clnum, %s FROM wcdma_results
        INNER JOIN platforms ON wcdma_results.platform_id=platforms.platform_id
        INNER JOIN testruns ON wcdma_results.testrun_id=testruns.testrun_id
        INNER JOIN wcdma_params ON wcdma_results.w_param_id=wcdma_params.w_param_id
        INNER JOIN testinfos ON wcdma_params.testinfo_id=testinfos.testinfo_id
        WHERE branch='%s'
            AND platform='%s'
            AND rfband=%s
            AND uarfcn=%s
            ...
            AND hspdsch_power=%s
            order by %s desc
            limit 1;""" % (varname, branch, platform, rfband, uarfcn, chtype, snr, datarate, txants, rfpower, schedtype, cpich_power, hspdsch_power, varname))
    logging.debug("EQUERY: %s" % equery_bestscore)
    return equery_bestscore


def DB_mySQL_wcdma_hspa_fading_SelectBestScore(host, dbname, uid, pwd, modeminfo, testparams):
    logging.debug(">> DB_mySQL_HSPA_Fading_SelectBestScore()...")
    dlthr_equery=DB_mySQL_wcdma_hspa_fading_SetQueryBestScore(modeminfo, testparams, 'dl_thput')
    dlthr_branch, dlthr_clnum, dlthr_score = DB_mySQL_export_bestscore(host=host, dbname=dbname, uid=uid, pwd=pwd, equery=dlthr_equery)

    dlthr_equery=DB_mySQL_wcdma_hspa_fading_SetQueryBestScore(modeminfo, testparams, 'dl_thput', rtype='QC_REF')
    dlthr_ref_branch, dlthr_ref_clnum, dlthr_ref_score = DB_mySQL_export_bestscore(host=host, dbname=dbname, uid=uid, pwd=pwd, equery=dlthr_equery)

    return (dlthr_ref_branch, dlthr_ref_clnum, dlthr_ref_score), (dlthr_branch, dlthr_clnum, dlthr_score)
...
class results_mySQLdb(object):
    ...
    def __init__ (self, host, dbname, uid, pwd, config_pwr_meas=1):

            # If database exists
            if 1:
                logging.debug("Remote database           : %s %s %s" % (host, dbname, uid))
                # Check connection
                self.host   = host
                self.name   = dbname
                self.uid    = uid
                ...
                self.config_pwr_meas = config_pwr_meas
            ...
            if 0:
                try:
                    ...
                    self.conn.executescript("""
                        CREATE TABLE IF NOT EXISTS platforms (platform_id     INTEGER PRIMARY KEY AUTO_INCREMENT,
                                                              platform        TEXT                NOT NULL,
                                                              aux_info        TEXT);

                        CREATE TABLE IF NOT EXISTS testinfos (testinfo_id     INTEGER PRIMARY KEY AUTO_INCREMENT,
                                                              rat             TEXT                NOT NULL,
                                                              testid          INTEGER UNSIGNED    NOT NULL,
                                                              descr           TEXT);

                        CREATE TABLE IF NOT EXISTS params (   param_id        INTEGER PRIMARY KEY AUTO_INCREMENT,
                        ...
                        CREATE TABLE IF NOT EXISTS testruns ( testrun_id      INTEGER PRIMARY KEY AUTO_INCREMENT,
                                                              timestamp       TIMESTAMP           NOT NULL,
                                                              branch          TEXT                NOT NULL,
                                                              clnum           INTEGER UNSIGNED    NOT NULL,
                                                              mod_files       INTEGER UNSIGNED    NOT NULL);

                        CREATE TABLE IF NOT EXISTS results (  result_id       INTEGER PRIMARY KEY AUTO_INCREMENT,
                        ...
                        CREATE TABLE IF NOT EXISTS wcdma_params (    w_param_id           INTEGER     PRIMARY KEY AUTO_INCREMENT,
                                                                     testinfo_id          INTEGER     NOT NULL,
                                                                     rfband               INTEGER     NOT NULL,
                        ...
                        CREATE TABLE IF NOT EXISTS wcdma_results (    w_result_id           INTEGER    PRIMARY KEY AUTO_INCREMENT,
                                                                      testrun_id          INTEGER    NOT NULL,
                                                                      platform_id         INTEGER    NOT NULL,
                        ...
                        CREATE TABLE IF NOT EXISTS pl1_db_info (     id            INTEGER    PRIMARY KEY AUTO_INCREMENT,
    ...
    def connect(self):
        if self.conn is None:
            ...
            logging.debug("Self.con is not None")
    ...
    def get_config_pwr_meas(self):
        return self.config_pwr_meas
...
#        self.c.execute("SELECT platform_id FROM platforms WHERE platform=? AND aux_info=?",(platform, aux_info))
    ...
    def add_platform(self, platform, aux_info=""):
        try:
            ...
            self.cursor.execute("INSERT INTO platforms(platform,aux_info) VALUES (%s,%s)", (platform,aux_info))
            logging.debug("Created platform record for %s...",(platform, aux_info))
            return self.get_platform_id(platform, aux_info)

    def get_testinfo_id(self, testid, rat):
        self.cursor.execute("SELECT testinfo_id FROM testinfos WHERE testid=%s AND rat=%s", (testid, rat))
        result=self.cursor.fetchone()[0]
        #if result is None:
    ...
    def add_testinfo(self, testid, descr, rat):
        try:
            ...
            self.cursor.execute("INSERT INTO testinfos(testid, rat, descr) VALUES (%s,%s,%s)", (testid, rat, descr))
            return self.get_testinfo_id(testid, rat)

    def get_testrun_id(self, timestamp, branch, clnum, mod_files):
        self.cursor.execute("SELECT testrun_id FROM testruns WHERE timestamp=%s AND branch=%s AND clnum=%s AND mod_files=%s", (timestamp, branch, clnum, mod_files))
        result=self.cursor.fetchone()[0]
    ...
    def add_testrun(self, timestamp, branch, clnum, mod_files):
        try:
            ...
            self.cursor.execute("INSERT INTO testruns(timestamp, branch, clnum, mod_files) VALUES (%s,%s,%s,%s)",(timestamp, branch, clnum, mod_files))
            return self.get_testrun_id(timestamp, branch, clnum, mod_files)
    ...
    def get_param_id (self, testinfo_id, rfband, earfcn, bwmhz, chtype, snr, rsepre, pa, pb, tm, txants, pmi, schedtype, dlmcs, dlnprb, dlrbstart, ulmcs, ulnprb, ulrbstart):
        self.cursor.execute("""SELECT param_id FROM params WHERE testinfo_id=%s AND rfband=%s AND earfcn=%s AND bwmhz=%s AND chtype=%s AND snr=%s AND rsepre=%s
                            AND pa=%s AND pb=%s AND tm=%s AND txants=%s AND pmi=%s AND schedtype=%s AND dlmcs=%s AND dlnprb=%s AND dlrbstart=%s AND ulmcs=%s AND ulnprb=%s AND ulrbstart=%s""",
    ...
    def add_param_set (self, testinfo_id, rfband, earfcn, bwmhz, chtype, snr, rsepre, pa, pb, tm, txants, pmi, schedtype, dlmcs, dlnprb, dlrbstart, ulmcs, ulnprb, ulrbstart):
        try:
            ...
            self.cursor.execute("INSERT INTO params(testinfo_id, rfband, earfcn, bwmhz, chtype, snr, rsepre, pa, pb, tm, txants, pmi, schedtype, dlmcs, dlnprb, dlrbstart, ulmcs, ulnprb, ulrbstart) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                      (testinfo_id, rfband, earfcn, bwmhz, chtype, snr, rsepre, pa, pb, tm, txants, pmi, schedtype, dlmcs, dlnprb, dlrbstart, ulmcs, ulnprb, ulrbstart))
            return self.get_param_id(testinfo_id, rfband, earfcn, bwmhz, chtype, snr, rsepre, pa, pb, tm, txants, pmi, schedtype, dlmcs, dlnprb, dlrbstart, ulmcs, ulnprb, ulrbstart)

    def get_result_id (self, testrun_id, platform_id, param_id):
        self.cursor.execute("SELECT result_id FROM results WHERE testrun_id=%s AND platform_id=%s AND param_id=%s",
    ...
    def add_result(self, testrun_id, platform_id, param_id, valid, dlrely, dlthr, dlbler, cqi, ack, nack, dtx, sf_total, sf_scheduled, ulrely, ulthr, ulbler, crc_pass, crc_fail, current_mA, rirely, numRI1, numRI2):
        try:
            ...
            self.cursor.execute("INSERT INTO results(testrun_id, platform_id, param_id, valid, dlrely, dlthr, dlbler, cqi, ack, nack, dtx, sf_total, sf_scheduled, ulrely, ulthr, ulbler, crc_pass, crc_fail, current_mA, rirely, numRI1, numRI2) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
    ...
    def get_wcdma_bler_perf_test_param_id (self, testinfo_id, rfband, uarfcn, chtype, datarate, snr, power, txant):
        self.cursor.execute("""SELECT w_param_id FROM wcdma_params WHERE testinfo_id=%s AND rfband=%s AND uarfcn=%s AND chtype=%s AND datarate=%s AND snr=%s AND power=%s
                            AND txant=%s""",
    ...
    def add_wcdma_bler_perf_test_param_set (self, testinfo_id, idx, table_entries):
        ...
        try:
            ...
            self.cursor.execute("INSERT INTO wcdma_params(testinfo_id, rfband, uarfcn, chtype, datarate, snr, power, txant) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)",
                      (testinfo_id, rfband, uarfcn, chtype, datarate, snr, power, txant))
            return self.get_wcdma_bler_perf_test_param_id(testinfo_id, rfband, uarfcn, chtype, datarate, snr, power, txant)


    def get_wcdma_result_id (self, testrun_id, platform_id, w_param_id):
        self.cursor.execute("SELECT w_result_id FROM wcdma_results WHERE testrun_id=%s AND platform_id=%s AND w_param_id=%s",
    ...
    def add_wcdma_bler_perf_test_result(self, testrun_id, platform_id, w_param_id, idx, table_entries):
        try:
            ...
            if self.get_config_pwr_meas():
                self.cursor.execute("INSERT INTO wcdma_results(testrun_id, platform_id, w_param_id, dlrely, dl_ber, dl_bler, lost_blocks, pdn_discontinuity, dl_verdict, i_min, i_avg, i_max, i_deviation, pwr_min, pwr_avg, pwr_max) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                ...
                self.cursor.execute("INSERT INTO wcdma_results(testrun_id, platform_id, w_param_id, dlrely, dl_ber, dl_bler, lost_blocks, pdn_discontinuity, dl_verdict) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                                   (testrun_id, platform_id, w_param_id, dlrely, dl_ber, dl_bler, lost_blocks, pdn_discontinuity, dl_verdict))
    ...
    def get_wcdma_hspa_test_param_id (self, testinfo_id, rfband, uarfcn, chtype, datarate, snr, power, txant, sched_type, modulation, ki, num_hsdsch_codes):
        self.cursor.execute("""SELECT w_param_id FROM wcdma_params WHERE testinfo_id=%s AND rfband=%s AND uarfcn=%s AND chtype=%s AND datarate=%s AND snr=%s AND power=%s
                            AND txant=%s AND sched_type=%s AND modulation=%s AND ki=%s AND num_hsdsch_codes=%s""",
    ...
    def add_wcdma_hspa_test_param_set (self, testinfo_id, idx, table_entries):
        ...
        try:
            ...
            self.cursor.execute("INSERT INTO wcdma_params(testinfo_id, rfband, uarfcn, chtype, datarate, snr, power, txant, sched_type, modulation, ki, num_hsdsch_codes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                      (testinfo_id, rfband, uarfcn, chtype, datarate, snr, power, txant, sched_type, modulation, ki, num_hsdsch_codes))
            return self.get_wcdma_hspa_test_param_id(testinfo_id, rfband, uarfcn, chtype, datarate, snr, power, txant, sched_type, modulation, ki, num_hsdsch_codes)
    ...
    def add_wcdma_hspa_test_result(self, testrun_id, platform_id, w_param_id, idx, table_entries):
        try:
            ...
            if self.get_config_pwr_meas():
                self.cursor.execute("INSERT INTO wcdma_results(testrun_id, platform_id, w_param_id, dl_bler, num_sf, dl_target_thput, dl_thput, tol, cqi, sent, ack, nack, dtx, dl_verdict, i_min, i_avg, i_max, i_deviation, pwr_min, pwr_avg, pwr_max ) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                ...
                self.cursor.execute("INSERT INTO wcdma_results(testrun_id, platform_id, w_param_id, dl_bler, num_sf, dl_target_thput, dl_thput, tol, cqi, sent, ack, nack, dtx, dl_verdict ) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                                    (testrun_id, platform_id, w_param_id, dl_bler, num_sf, dl_target_thput, dl_thput, tol, cqi, sent, ack, nack, dtx, dl_verdict))
            return self.get_wcdma_result_id(testrun_id, platform_id, w_param_id)
    ...
    def get_wcdma_dc_hspa_test_param_id (self, testinfo_id, rfband, uarfcn, chtype, datarate, snr, power, txant, sched_type, modulation, ki, num_hsdsch_codes, snr_2, power_2, modulation_2, ki_2, num_hsdsch_codes_2):
        self.cursor.execute("""SELECT w_param_id FROM wcdma_params WHERE testinfo_id=%s AND rfband=%s AND uarfcn=%s AND chtype=%s AND datarate=%s AND snr=%s AND power=%s
                            AND txant=%s AND sched_type=%s AND modulation=%s AND ki=%s AND num_hsdsch_codes=%s AND snr_2=%s AND power_2=%s AND modulation_2=%s AND ki_2=%s AND num_hsdsch_codes_2=%s""",
    ...
    def add_wcdma_dc_hspa_test_param_set (self, testinfo_id, idx, table_entries):
        ...
        try:
            ...
            self.cursor.execute("INSERT INTO wcdma_params(testinfo_id, rfband, uarfcn, chtype, datarate, snr, power, txant, sched_type, modulation, ki, num_hsdsch_codes, snr_2, power_2, modulation_2, ki_2, num_hsdsch_codes_2) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                      (testinfo_id, rfband, uarfcn, chtype, datarate, snr, power, txant, sched_type, modulation, ki, num_hsdsch_codes, snr_2, power_2, modulation_2, ki_2, num_hsdsch_codes_2))
            return self.get_wcdma_dc_hspa_test_param_id(testinfo_id, rfband, uarfcn, chtype, datarate, snr, power, txant, sched_type, modulation, ki, num_hsdsch_codes, snr_2, power_2, modulation_2, ki_2, num_hsdsch_codes_2)
    ...
    def add_wcdma_dc_hspa_test_result(self, testrun_id, platform_id, w_param_id, idx, table_entries):
        try:
            ...
            # power measurements may be disabled, therefore need to check for
            # table entries
            if self.get_config_pwr_meas():
                i_min               = table_entries['Imin[mA]'][idx]
                i_avg               = table_entries['Iavrg[mA]'][idx]
            ...
            if self.get_config_pwr_meas():
                self.cursor.execute("INSERT INTO wcdma_results(testrun_id, platform_id, w_param_id, dl_bler, num_sf, dl_target_thput, dl_thput, tol, cqi, sent, ack, nack, dtx, dl_target_thput_2, dl_thput_2, dl_bler_2, cqi_2, sent_2, ack_2, nack_2, dtx_2, dl_verdict, i_min, i_avg, i_max, i_deviation, pwr_min, pwr_avg, pwr_max ) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                ...
                self.cursor.execute("INSERT INTO wcdma_results(testrun_id, platform_id, w_param_id, dl_bler, num_sf, dl_target_thput, dl_thput, tol, cqi, sent, ack, nack, dtx, dl_target_thput_2, dl_thput_2, dl_bler_2, cqi_2, sent_2, ack_2, nack_2, dtx_2, dl_verdict ) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                                   (testrun_id, platform_id, w_param_id, dl_bler, num_sf, dl_target_thput, dl_thput, tol, cqi, sent, ack, nack, dtx, dl_target_thput_2, dl_thput_2, dl_bler_2, cqi_2, sent_2, ack_2, nack_2, dtx_2, dl_verdict))

            return self.get_wcdma_result_id(testrun_id, platform_id, w_param_id)


    def get_wcdma_hspa_fading_test_param_id (self, testinfo_id, rfband, uarfcn, chtype, datarate, snr, power, txant, sched_type, cpich_power, hspdsch_power):
        self.cursor.execute("""SELECT w_param_id FROM wcdma_params WHERE testinfo_id=%s AND rfband=%s AND uarfcn=%s AND chtype=%s AND datarate=%s AND snr=%s AND power=%s
                            AND txant=%s AND sched_type=%s AND cpich_power=%s AND hspdsch_power=%s""",
                            (testinfo_id, rfband, uarfcn, chtype, datarate, snr, power, txant, sched_type, cpich_power, hspdsch_power))
        result=self.cursor.fetchone()[0]
        logging.debug("WCDMA HSPA Fading Param set %s has ID %d",(testinfo_id, rfband, uarfcn, chtype, datarate, snr, power, txant, sched_type, cpich_power, hspdsch_power), result)
        return result

    def add_wcdma_hspa_fading_test_param_set (self, testinfo_id, idx, table_entries):
        rfband              = table_entries['RFBAND'][idx]
        uarfcn              = table_entries['UARFCN'][idx]
        chtype              = table_entries['CHTYPE'][idx]
        datarate            = table_entries['DATARATE'][idx]
        ...
        txant               = table_entries['TXANTS'][idx]
        sched_type          = table_entries['SCHEDTYPE'][idx]
        cpich_power         = table_entries['CPICH_Power'][idx]
        hspdsch_power       = table_entries['HS-PDSCH_Power'][idx]

        try:
            return self.get_wcdma_hspa_fading_test_param_id(testinfo_id, rfband, uarfcn, chtype, datarate, snr, power, txant, sched_type, cpich_power, hspdsch_power)
        except TypeError:
            logging.debug("Creating record for wcdma HSPA Fading params set %s",(testinfo_id, rfband, uarfcn, chtype, datarate, snr, power, txant, sched_type,cpich_power, hspdsch_power))
            self.cursor.execute("INSERT INTO wcdma_params(testinfo_id, rfband, uarfcn, chtype, datarate, snr, power, txant, sched_type, cpich_power, hspdsch_power) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                      (testinfo_id, rfband, uarfcn, chtype, datarate, snr, power, txant, sched_type, cpich_power, hspdsch_power))
            return self.get_wcdma_hspa_fading_test_param_id(testinfo_id, rfband, uarfcn, chtype, datarate, snr, power, txant, sched_type, cpich_power, hspdsch_power)

    def add_wcdma_hspa_fading_test_result(self, testrun_id, platform_id, w_param_id, idx, table_entries):
        try:
            return self.get_wcdma_result_id(testrun_id, platform_id, w_param_id)
        except TypeError:
            num_sf              = table_entries['NSF'][idx]
            dl_target_thput     = table_entries['REF Tput(Mbps)'][idx]
            dl_thput            = table_entries['DL_TPUT(Mbps)'][idx]
            tol                 = table_entries['TOL(%)'][idx]
            cqi                 = table_entries['CQI'][idx]
            sent                = table_entries['SENT(%)'][idx]
...
if __name__ == '__main__':
    ...
    #root_dir           =os.sep.join(os.environ['PL1_WCDMA_TEST_ROOT'].split(os.sep)[:])
    root_dir           =os.sep.join(os.environ['PL1TESTBENCH_ROOT_FOLDER'].split(os.sep)[:]+['wcdma'])
    database_dir       =os.sep.join(root_dir.split(os.sep)[:]+['common','report','sqllite', 'database'])
    equery_dir         =os.sep.join(root_dir.split(os.sep)[:]+['common','report','sqllite', 'database'])
    log_file           =os.sep.join(root_dir.split(os.sep)[:]+['common','report','sqllite', 'database','perf_bestscore_db.LOG'])
    ...
    logger_test=logging.getLogger('run_DB_mySQL_Test')
    ...
    if 0:
        print("DB SCHEMA    :")
        db_h=results_mySQLdb(host='localhost', dbname='pl1testbench', uid='root', pwd=pwd)
        db_h.cursor.execute("""show tables""")
        cursor_l = db_h.cursor.fetchall()
        #cursor_l = db_h.cursor.execute(".tables")
        if cursor_l:
            for cursor in cursor_l:
                print 'TABLE: %s' % (cursor[0])
                #db_h.cursor.execute("describe %s" % (cursor[0]))
                #des = db_h.cursor.description
                #print "%s", (des)
    ...
    if 0:
        ...
        DB_mySQL_export_to_file(dbname=db_file, equery=EQUERY_DATABASE, filename=EQUERY_DATABASE_FILE)
    ...
    if 0:
        ...
        #DB_mySQL_export_to_file(dbname=db_file, equery=EQUERY_BESTSCORE, filename=EQUERY_BESTSCORE_FILE)
        branch,clnum,score = DB_mySQL_export_bestscore(dbname=db_file, equery=EQUERY_BESTSCORE)
        print branch
        print clnum
        print score
    ...
    if 0:
        ...
        uid     = 'root'
        pwd     = 'BZh91AY&SY!-o\xae\x00\x00\x01\x01\x80\x00\x00\x94\x00 \x000\xcc\x0c\xf5\x06qw$S\x85\t\x02\x12\xd6\xfa\xe0'
        print ">> DB_mySQL_W_import_from_file_meas ....."
        ...
        #DB_mySQL_wcdma_import_from_file_meas (filename=filename, testType='BLER_PERF', host=host, dbname=dbname, uid=uid, pwd=pwd )
        ...
        #DB_mySQL_wcdma_import_from_file_meas (filename=filename, testType='HSPA_BLER_PERF', host=host, dbname=dbname, uid=uid, pwd=pwd)
        ...
        #DB_mySQL_wcdma_import_from_file_meas (filename=filename, testType='DCHSDPA_BLER_PERF', host=host, dbname=dbname, uid=uid, pwd=pwd)

        filename = "/home/jsorathia/Perforce/jsorathia_ubuntu_812/software/teams/phy/pl1_dev.br/modem/l123/phy/test/pl1_wcdma_testbench/results/20140721_151401_WCDMA_TestReport/WCDMA_CMW500_TestReport_testID_101_testType_HSPA_FADING_PERF.csv"
        DB_mySQL_wcdma_import_from_file_meas (filename=filename, testType='HSPA_FADING_PERF', host=host, dbname=dbname, uid=uid, pwd=pwd)
    ...
    if 0:
        ...
        uid     = 'root'
        pwd     = 'BZh91AY&SY!-o\xae\x00\x00\x01\x01\x80\x00\x00\x94\x00 \x000\xcc\x0c\xf5\x06qw$S\x85\t\x02\x12\xd6\xfa\xe0'
        DB_mySQL_CheckPermission(host, dbname, uid, pwd)
